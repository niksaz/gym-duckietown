...
class Stats:
    ...
    def __init__(self):
        self.cnt = 0
        self.col_penalty = StatsFinder()
        self.speed = StatsFinder()
        self.lp_dist = StatsFinder()
        self.lp_dot_dir = StatsFinder()
        self.lp_angle_deg = StatsFinder()
        self.lp_angle_rad = StatsFinder()
        self.reward = StatsFinder()

    def process(self, lp, col_penalty, speed, reward):
        self.cnt += 1
        if self.cnt % 1000 == 0:
            print('After', self.cnt, 'steps')
            print('col_penalty', end=': ')
            self.col_penalty.report()
            print('speed', end=': ')
            self.speed.report()
            print('lp_dist', end=': ')
            self.lp_dist.report()
            print('lp_dot_dir', end=': ')
            self.lp_dot_dir.report()
            print('lp_angle_deg', end=': ')
            self.lp_angle_deg.report()
            print('lp_angle_rad', end=': ')
            self.lp_angle_rad.report()
            print('reward', end=': ')
            self.reward.report()

        self.col_penalty.account_for(col_penalty)
        self.speed.account_for(speed)
        self.lp_dist.account_for(lp.dist)
        self.lp_dot_dir.account_for(lp.dot_dir)
        self.lp_angle_deg.account_for(lp.angle_deg)
        self.lp_angle_rad.account_for(lp.angle_rad)
        self.reward.account_for(reward)
...
class Simulator(gym.Env):
    ...
    def compute_reward(self):
        ...
        angle = math.atan2(-delta_pos[2], delta_pos[0])
        if angle < 0:
            angle += 2 * math.pi
        ...
        # print('reward', reward)
        # stats.process(lp, col_penalty, speed, reward)
